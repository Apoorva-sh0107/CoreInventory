...
import { useInventory, DeliveryDocument, Product } from '@/context/InventoryContext';
...
export default function DeliveriesPage() {
  ...
  const { products, deliveries, addDeliveryDoc, updateDeliveryDocStatus, warehouses, loading: invLoading } = useInventory();
  const router = useRouter();
  ...
  // Form states for New Delivery Document
  ...
  const filteredDeliveries = useMemo(() => {
    return (deliveries || []).filter(d => d.id.toLowerCase().includes(search.toLowerCase()) || d.customer.toLowerCase().includes(search.toLowerCase()));
  }, [deliveries, search]);

  const handleCreateDelivery = (e: React.FormEvent) => {
    e.preventDefault();
    if (!customer || !date || !warehouseCode || items.length === 0) return;
    ...
    const count = (deliveries || []).filter(d => d.id.includes(warehouseCode)).length + 1;
    const refId = `${warehouseCode}/OUT/${count.toString().padStart(4, '0')}`;
    ...
    setCustomer(''); setDate(''); setWarehouseCode(''); setItems([{ productId: '', quantity: '' }]);
  };

  const handleAddItem = () => setItems([...items, { productId: '', quantity: '' }]);

  const checkStockAvailability = (itemsList: any[]) => {
    let sufficient = true;
    itemsList.forEach(item => {
      const product = products.find(p => p.id === Number(item.productId));
      if (!product || product.freeToUse < Number(item.quantity)) sufficient = false;
    });
    return sufficient;
  };
      ...
      <div style={{ width: '100%', display: 'flex', justifyContent: 'center', padding: '48px 32px' }}>
        <main style={{ width: '100%', maxWidth: '1200px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <h1 style={{ fontSize: '28px', fontWeight: '800', color: '#1A1A2E' }}>Deliveries</h1>
              <p style={{ fontSize: '14px', color: '#8E8E9A', marginTop: '4px' }}>Manage outgoing structure operation validation buffers flawless layouts.</p>
            </div>
            <div style={{ display: 'flex', gap: '12px' }}>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search deliveries…" style={{ ...inputStyle, width: '220px' }} />
              <button onClick={() => setIsNewModalOpen(true)} style={{ padding: '12px 18px', background: 'linear-gradient(135deg,#714B67,#9D6B8F)', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '700', cursor: 'pointer' }}>Create Delivery</button>
            </div>
          </div>
          ...
          <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #E8E4ED', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead style={{ background: '#FAF8FC' }}>
                <tr>{['Reference', 'Customer', 'Warehouse', 'Scheduled Date', 'Status', 'Actions'].map(h => <th key={h} style={{ padding: '14px 16px', fontSize: '11px', fontWeight: '800', color: '#8E8E9A', textTransform: 'uppercase', textAlign: 'left', borderBottom: '1px solid #F0EDF2' }}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {filteredDeliveries.map(d => {
                  const isAvailable = checkStockAvailability(d.items);
                  const isLate = new Date(d.date) < new Date() && (d.status as string) !== 'Done';

                  return (
                    <tr key={d.id} style={{ borderBottom: '1px solid #F8F5FB' }}>
                      <td style={{ padding: '14px 16px', fontSize: '13.5px', fontWeight: '800', color: '#714B67' }}>{d.id}</td>
                      <td style={{ padding: '14px 16px', fontSize: '14px', fontWeight: '600' }}>{d.customer}</td>
                      <td style={{ padding: '14px 16px', fontSize: '13.5px', color: '#64647A' }}>{d.warehouseCode}</td>
                      <td style={{ padding: '14px 16px', fontSize: '13.5px', color: '#64647A' }}>{d.date}</td>
                      <td>
                        <span style={{ 
                          fontSize: '11px', fontWeight: '800', padding: '4px 8px', borderRadius: '6px', 
                          background: d.status === 'Done' ? '#E1FCEF' : isLate ? '#FEE2E2' : !isAvailable && (d.status as string) !== 'Done' ? '#FEF3C7' : '#EFF6FF', 
                          color: d.status === 'Done' ? '#059669' : isLate ? '#DC2626' : !isAvailable && (d.status as string) !== 'Done' ? '#D97706' : '#2563EB' 
                        }}>
                          {d.status === 'Done' ? 'Done' : isLate ? 'Late' : !isAvailable ? 'Waiting' : 'Ready'}
                        </span>
                      </td>
                      <td style={{ padding: '14px 16px', display: 'flex', gap: '8px' }}>
                        {d.status === 'Draft' && <button onClick={() => updateDeliveryDocStatus(d.id, isAvailable ? 'Ready' : 'Waiting')} style={{ padding: '6px 12px', background: '#EFF6FF', color: '#2563EB', border: 'none', borderRadius: '6px', fontWeight: '700', cursor: 'pointer', fontSize: '12px' }}>Validate</button>}
                        {(d.status === 'Ready' || (d.status === 'Waiting' && isAvailable)) && <button onClick={() => updateDeliveryDocStatus(d.id, 'Done')} style={{ padding: '6px 12px', background: '#E1FCEF', color: '#059669', border: 'none', borderRadius: '6px', fontWeight: '700', cursor: 'pointer', fontSize: '12px' }}>Deliver</button>}
                        {(!isAvailable && d.status === 'Waiting') && <span style={{ fontSize: '12px', color: '#DC2626', fontWeight: '700' }}>Insufficient Stock</span>}
                      </td>
                    </tr>
                  );
                })}
            ...
            {isNewModalOpen && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} style={{ background: 'white', padding: '32px', borderRadius: '20px', width: '500px', display: 'flex', flexDirection: 'column', gap: '16px', boxShadow: '0 20px 50px rgba(0,0,0,0.15)' }}>
                  <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '800' }}>Create New Delivery</h2>
                  
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div><label style={{ fontSize: '12px', fontWeight: '700', color: '#8E8E9A' }}>Customer</label><input value={customer} onChange={e => setCustomer(e.target.value)} style={inputStyle} /></div>
                    ...
                    <div style={{ marginTop: '8px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '150px', overflowY: 'auto' }}>
                        {items.map((item, idx) => {
                          const product = products.find(p => p.id === Number(item.productId));
                          const insufficient = product && product.freeToUse < Number(item.quantity);

                          return (
                            <div key={idx} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                              <select value={item.productId} onChange={e => { const u = [...items]; u[idx].productId = e.target.value; setItems(u); }} style={{ ...inputStyle, flex: 2, border: insufficient ? '1.5px solid #DC2626' : '1.5px solid #E0DDE3' }}>
                                <option value="">Select Item...</option>
                                {products.map(p => <option key={p.id} value={p.id}>{p.name} (Available: {p.freeToUse})</option>)}
                              </select>
                              <input type="number" placeholder="Qty" value={item.quantity} onChange={e => { const u = [...items]; u[idx].quantity = e.target.value; setItems(u); }} style={{ ...inputStyle, flex: 1, border: insufficient ? '1.5px solid #DC2626' : '1.5px solid #E0DDE3' }} />
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
                    ...
                    <button onClick={handleCreateDelivery} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg,#714B67,#9D6B8F)', color: 'white', fontWeight: '700', cursor: 'pointer' }}>Save Draft</button>
                  </div>
                </motion.div>
              </motion.div>
            )}
